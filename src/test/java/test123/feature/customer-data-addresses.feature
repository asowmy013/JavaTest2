...
@user-profile-management  @customer-data-addresses
Feature: GET /v1/customer-data/{UUID}/addresses
  Get addresses data of Customer
  ...
  Scenario: Validate Get Personal Data Endpoint with valid UUID
    ...
    When request submitted to get customer addresses endpoint with "global" UUID
  ...
  Scenario: Validate Get Personal Data Endpoint with invalid UUID Type
    Given a customer "USER_WITH_VALID_GLOBAL_UUID"
    When request submitted to get customer addresses endpoint with "local" UUID
